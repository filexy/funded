...
/*
 ...
 |--------- -------------------------
 */
Route::get('donate/{id}/{slug?}',[ 'uses' =>'DonationsController@show']);
...
Route::get('donate_paynow', 'PaynowController@pay')->name('paynow');
Route::get('paydonation/{id}/{email}', function($id,$email){
    DB::update('update donations set approved=? WHERE campaign_id=?',[1,$id]);
});

Route::get('success/{email}', function($email){

    ;
    $subject ="Donation Payment Successful";
    $txt = "This email is to confirm that your donation to Funded Was recieved Successfully";
    $headers = "From: noreply@funded.co.zw" . "\r\n";

    mail($email,$subject,$txt,$headers);
    return view('default.donate')->with(['success'=>'Owner Added Successfully']);
